...
class MutiModalDataset(Dataset):
    def __init__(self,data_path,video_feat_path,cata1_num,cata2_num,bert_path,id_weight2=None,id_weight1=None,num_worker=8,max_token_lens=[128,128,128],max_frame=32,test=False,seed=1000):
        self.datas=[]
        self.test=test
        self.video_feat_path=video_feat_path
        self.cata1_num=cata1_num
        self.cata2_num=cata2_num
        ...
        self.id_weight2=id_weight2
        self.id_weight1=id_weight1
        rnd.seed(seed)
        if not test:
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(ocr)):
            ...
            if cur_ocr not in ocr_map:
                ...
                ocr_s += cur_ocr
        cata=data['category_id']
        cata1=category_id_map.category_id_to_lv1id(cata)
        cata2=category_id_map.category_id_to_lv2id(cata)

        label1=torch.zeros(1,self.cata2_num,dtype=torch.float32)
        label2=torch.zeros(1,self.cata2_num,dtype=torch.float32)
        ids=category_id_map.get_all_lv1id_index(cata)
        #print(cata,ids)
        for i in range(len(ids)):
            label1[0][ids[i]]=1
        #print(label1)
        
        if self.id_weight2 is not None:
            weight=self.id_weight2[cata]
            label2[0][cata2] = weight
        else:
            label2[0][cata2]=1
