...
def get_cata_weight(path):
    ...
    for line in json_file:
        cata_id=line['category_id']
        if cata_id not in id_states2:
            id_states2[cata_id]=1
        else:
            id_states2[cata_id]+=1
        lv1_id=cata_id[:2]
        if lv1_id not in id_states1:
            id_states1[lv1_id]=1
        else:
            id_states1[lv1_id]+=1
        cnt+=1
    for cata_id in id_states2:

        id_weight2[cata_id]=math.log(cnt/id_states2[cata_id])
        min_val = min(min_val,id_weight2[cata_id])
        max_val=max(max_val,id_weight2[cata_id])
    for lv1_id in id_states1:
        id_weight1[lv1_id]=math.log(cnt/id_states1[lv1_id])/4
    for cata_id in id_weight2:
        id_weight2[cata_id]/=8
    return id_weight2,id_weight1
...
def get_all_lv1id_index(category_id):
    lv1id=category_id[:2]
    ids=[]
    for k in LV2ID_TO_CATEGORY_ID:
        id=LV2ID_TO_CATEGORY_ID[k]
        if id[:2]==lv1id:
            ids.append(k)
    return ids
